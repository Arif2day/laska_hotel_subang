...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Data Menu Type</h1>
    </div>
        ...
        <div class="col-xl-12 col-md-12 col-lg-12 mb-1">
            <div class="card shadow mb-4">
                ...
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    <h6 class="m-0 font-weight-bold text-primary">Data Menu Type</h6>
                </div>
                <!-- Card Body -->
                <div class="card-body">
                    <div class="row float-right mr-0 mb-2">
                        ...
                        <button class="btn btn-sm btn-info" data-toggle="modal" data-target="#addMenuTypeModal">
                            <i class="fas fa-fw fa-plus-circle"></i> Add Menu Type</button>
                    </div>
                    <div class="text-xs">
                        <table class="table menu-type-datatable display" style="width:100%;">
                        ...
                        <input type="hidden" id="urllist" name="urllist" value="{{url('master/menu-type/list')}}">
                        <input type="hidden" id="urldel" name="urldel" value="{{url('master/menu-type')}}">                        
                    </div>
                </div>
            </div>
        </div>
    ...
    @include('Admin.SUPER.menu-type.modal.add')
    @include('Admin.SUPER.menu-type.modal.edit')
...
<script>
    ...
    var table = $('.menu-type-datatable').DataTable({   
        pageLength : 25,
        dom: 'lfrtip',        
        processing: true,
        serverSide: true,
    ...
    function deleteMenuType(params) {
        Swal.fire({
            title: 'Yakin?',
            text: "Anda akan menghapus Data Menu Type.",
            icon: 'warning',
    ...
    // Form Sort Function
    function clearFormMenuType() {
        document.getElementById('name').value='';
    } 

    function clearEditFormMenuType() {
    ...
    function saveMenuType() {
        ...
        if (name=="") {
            Swal.fire({icon: 'error', title: 'Oops...',text: "Type Required!",});
        }
        else{
            let datar = {};
            datar['_method']='POST';
    ...
    $(document).on('click', '.editMenuTypeBtn', function () {
        // Ambil data dari atribut tombol
        var id = $(this).data('id');
        var name = $(this).data('type');

        // Set data ke form dalam modal
        $('#editMenuTypeModal input[id="e_id"]').val(id);
        $('#editMenuTypeModal input[id="e_name"]').val(name);
    });
</script>
@endsection